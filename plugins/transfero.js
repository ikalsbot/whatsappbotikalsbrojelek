            ...
            case 'money':
                if (global.db.data.users[m.sender].money = count * 1) {
                    try {
                        ...
                        conn.reply(who, `Selamat mendapatkan money sebesar ${count}\nFrom: *Owner*`.trim(), m)
                    } catch (e) {
            ...
            case 'exp':
                if (global.db.data.users[m.sender].exp = count * 1) {
                    try {
                        ...
                        conn.reply(who, `Selamat mendapatkan exp sebesar ${count}\nFrom: *Owner*`.trim(), m)
                    } catch (e) {
                ...
                break
            case 'join':
                if (global.db.data.users[m.sender].joinlimit = count * 1) {
                    try {
                        global.db.data.users[who].joinlimit += count * 1
                        conn.reply(m.chat, `Berhasil mentransfer joinlimit sebesar ${count}`.trim(), m)
                        conn.reply(who, `Selamat mendapatkan joinlimit sebesar ${count}\nFrom: *Owner*`.trim(), m)
                    } catch (e) {
                        global.db.data.users[m.sender].joinlimit += count * 1
                        m.reply('Gagal Menstransfer')
                        console.log(e)
                        if (owner) {
                            for (let jid of global.owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != conn.user.jid)) {
                ...
                } else conn.reply(m.chat, `joinlimit kamu tidak mencukupi untuk mentransfer joinlimit sebesar ${count}`.trim(), m)
                break
             case 'limit':
