...
let handler = async (m, { conn, args, usedPrefix, isOwner }) => {
    ...
    try {
        ...
        let _waktutionskh = (15000 - __waktutionskh)
        let waktutionskh = clockString(_waktutionskh)
        if (new Date - global.db.data.users[m.sender].judilast > 15000) {
...
handler.help = ['judip <jumlah> (winrate 40%)']
handler.tags = ['advanced']
handler.command = /^(judip)$/i
handler.limit = true
handler.group = true
