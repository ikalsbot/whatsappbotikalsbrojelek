...
let handler = async (m, { conn, args, usedPrefix, isOwner }) => {
    ...
    try {
        ...
        let _waktutionskh = (10000 - __waktutionskh)
        let waktutionskh = clockString(_waktutionskh)
        if (new Date - global.db.data.users[m.sender].judilast > 10000) {
        global.db.data.users[m.sender].judilast = new Date * 1
        let randomaku = `${Math.floor(Math.random() * 50)}`.trim()
        let randomkamu = `${Math.floor(Math.random() * 45)}`.trim()                //hehe Biar Susah Menang :v
        let Aku = (randomaku * 1)
        ...
        if (global.db.data.users[m.sender].money >= count * 1) {
          ...
          //  await m.reply('*Jangan judi gk bakal menang!!, kalau gk percaya gpp*') //Kwkwwkkwlwlw
            if (Aku > Kamu) {
                conn.reply(m.chat, `🤖ikalsbot roll:${Aku}\n🧔Kamu roll: ${Kamu}\n\nkamu *KALAH*, kamu kehilangan ${count} money`.trim(), m)
            } else if (Aku < Kamu) {
                global.db.data.users[m.sender].money += count * 2
                conn.reply(m.chat, `🤖ikalsbot roll:${Aku}\n🧔kamu roll: ${Kamu}\n\nkamu *MENANG*, kamu Mendapatkan\n ${count * 2} money\nMoney telah Dimasukan kedalam dompetmu`.trim(), m)
            } else {
                global.db.data.users[m.sender].money += count * 1
    ...
    conn.sendButton(m.chat, '','DOMPET', wm, 'MENU', '.menu', m)
}
 }
